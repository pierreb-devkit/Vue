...
- [2026-08-06] billing: guarding a "no quota, no overage" invariant at each call site (`quota > 0 ? overage : 0`) instead of inside the shared component that owns the pinned/error branch -> move the neutralisation inside the component, evaluated ahead of every branch that reads it; then grep every consumer for a duplicate raw read of the same value outside the delegated child (one consumer had a second, local unguarded read driving its own text color, missed by only checking the prop passed to the child). See #4553.
- [2026-09-25] auth: persisting a post-auth `?redirect=` in `sessionStorage` so it survives past the signup form submit -> the email-verification link opens in a NEW TAB, where `sessionStorage` (scoped to the tab that wrote it) is empty; use `localStorage` instead: single-use, 24h TTL, and cleared on every signup/signin mount whose `?redirect=` is absent or unsafe — without that clear, a record planted by an attacker link outlives its author and redirects the next unrelated person who authenticates on the same browser. See `src/modules/auth/lib/postAuthRedirect.js`, #4675.
- [2026-09-26] docs-seo: llms.txt `(Markdown)` twin built from the frontend page URL (`{app.url}{basePath}/{cat}/{slug}.md`) -> that route serves the SPA HTML shell (200), not markdown; build the twin from `docs.contentUrl` instead (`{contentUrl}/{slug}.md`, slug only, no category segment), matching the encoding `deriveDocsSnapshotEntries` already uses for the same endpoint. A missing/malformed `contentUrl` omits the twin rather than emit a wrong link. See #4683.
