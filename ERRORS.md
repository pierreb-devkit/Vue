...
- [2026-09-25] auth: persisting a post-auth `?redirect=` in `sessionStorage` so it survives past the signup form submit -> the email-verification link opens in a NEW TAB, where `sessionStorage` (scoped to the tab that wrote it) is empty; use `localStorage` instead, single-use (cleared on first read) with a TTL, so a stale record never resurfaces for an unrelated later session. See `src/modules/auth/lib/postAuthRedirect.js`, #4675.
