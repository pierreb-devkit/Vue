 ...
 *   across test navigations in the same browser context, which can corrupt the auth/config
...
async function injectFakeAuth(page) {
  ...
  // Layer 3: page-level addInitScript fetch stub — fires before any page script on the
  // specific page. Using page.addInitScript (not page.context().addInitScript) avoids
  // script accumulation across test navigations in the same context. Axios 1.x uses the
  // fetch adapter by default in modern Chromium; this stub intercepts refreshAbilities()
  // before the Playwright CDP network layer activates for the new document.
  await page.addInitScript((body) => {
    const origFetch = window.fetch;
    // biome-ignore lint/correctness/useQwikValidLexicalScope: false positive — Qwik rule does not apply in a Vue/Playwright context
    window.fetch = async (input, init) => {
